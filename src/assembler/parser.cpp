...
Parser::Parser() 
    ...
    , phaseOrigin_(0)
    , phaseOffset_(0)
    , operandAnalyzer_(instructions_, symbolTable_)
        ...
        if (token.type == TokenType::Identifier) {
            ...
            if (upperMnemonic == "ORG") {
                ...
                if (upperMnemonic == "DB") {
                    ...
                    parsedLine.operandString = operandAnalyzer_.parseOperands(lexer, parsedLine.operands);
                    ...
                    auto info = operandAnalyzer_.findInstructionVariant(parsedLine.mnemonic, parsedLine.operands);
                    if (info) {
...
bool Parser::generateInstruction(ParsedLine& line, const std::string& /* filename */) {
    ...
    auto info = operandAnalyzer_.findInstructionVariant(line.mnemonic, line.operands);
    if (info) {
        ...
        if (info->operandBytes > 0) {
            ...
            for (const std::string& op : line.operands) {
                ...
                if (operandAnalyzer_.isRegisterOperand(upper)) continue;
